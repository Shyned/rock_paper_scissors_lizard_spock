...
class Human_Vs_Human(Player):
    def __init__(self):
        ...
        self.player1_name = ''
        self.player2_name = ''
        self.pick_name_player1()
        self.pick_name_player2()
    ...
    def players_pick(self):
        ...
        while self.player1.lower() not in self.choice:
            print(self.choice)
            self.player1 = input(
                "Player 1 please enter a choice from the list: ")
        else:
            # if player one pick is in list then player 2 pick
            while self.player2 not in self.choice:
                self.player2 = input(
                    "Player 2 please enter a choice from the list: ")

            else:
                pass

    def pick_name_player1(self):
        while len(self.player1_name) < 3:
            self.player1_name = input('Print your name here: ')
        else:
            pass

    def pick_name_player2(self):
        while len(self.player2_name) < 3:
            self.player2_name = input('Print your name here: ')
        else:
            pass
    ...
    def repeat_till_over(self):
        while True:
            if self.player1_points < 2 and self.player2_points < 2:
                ...
                print(f"{self.player2_name} is the winner")
                quit()
